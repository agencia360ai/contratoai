...
export async function generateMetadata({
  ...
  const who = link.display_name || "TeContrato Panamá";
  return {
    title: `Sube tu CV con ${who}`,
    description:
      "Sube tu CV en PDF, foto o cuéntanos en audio sobre ti. La IA arma tu perfil para que las empresas te encuentren.",
  };
}
...
export default async function CandidateIntakePage({
  ...
  const link = await loadLink(slug);
  if (!link) notFound();
  if (link.kind === "job") notFound();

  return (
    <main className="relative min-h-screen pt-12 pb-24">
      <div className="absolute inset-0 bg-grid-light" aria-hidden />
      <div
        className="absolute left-1/2 top-0 -translate-x-1/2 h-[400px] w-[700px] rounded-full bg-gradient-to-br from-violet-100/40 to-transparent blur-3xl opacity-30"
        aria-hidden
      />
